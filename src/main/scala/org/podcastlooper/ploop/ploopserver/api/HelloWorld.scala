...
final class HelloWorld[F[_]: Concurrent: ContextShift: Sync: Timer] extends Http4sDsl[F] {
  ...
  private val sayHello: HttpRoutes[F] = Http4sServerInterpreter.toRoutes(HelloWorld.greetings) { name =>
    val greetings = (
